...
- (void)onLogoutClick
{
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"確認" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        ...
        // 將自動登入取消,不然會無法進入登入畫面
        [[NSUserDefaults standardUserDefaults] setBool:NO forKey:YSIsAutoLogin];
        [[NSUserDefaults standardUserDefaults] synchronize];
        
        // 通知Login介面從UserDefaults讀取新的設定並且更新UI
        [[NSNotificationCenter defaultCenter] postNotificationName:YSMessage_onBackToLoginView object:self];
